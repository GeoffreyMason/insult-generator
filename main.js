...
function generateCurseWord() {
  const curseWord = randomWord(insults);

  document.getElementById('curse-word').innerHTML = curseWord;
  generateInsultSeverity();
}
...
function generateInsultSeverity() {
  const insultSeverities = ['Minor', 'Moderate', 'Serious', 'Critical'];
  const insultColours = ['#9EE493', '#f0d76a', '#F0B66A', '#F25767'];
  const severityAmount = Math.min(
    insultSeverities.length,
    insultColours.length
  );

  const severityIndex = randomNumber(0, severityAmount);
  ...
  meter.style.width = ((severityIndex + 1) / severityAmount) * 100 + '%';
